...
def query_backend(book_id: str, query: str):
    ...
    try:
        ...
        return result.answer, str(result.sources)
    except Exception as e:
        return f"Error processing query: {str(e)}", "[]"

# Create Gradio interface
with gr.Blocks(title="RAG Chatbot Backend") as demo:
    ...
    with gr.Row():
        ...
        with gr.Column():
            ...
            sources_output = gr.Textbox(label="Sources", interactive=False, lines=8)
    ...
    gr.Markdown("- `/api/query` - POST endpoint for querying")
    gr.Markdown("- `/api/ingest` - POST endpoint for ingesting books")
    gr.Markdown("- `/api/health` - GET endpoint for health checks")

# For Hugging Face Spaces, we typically run with gradio
